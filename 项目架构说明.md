# Table Tool 项目架构说明

## 📋 目录
- [项目概述](#项目概述)
- [技术栈](#技术栈)
- [架构设计](#架构设计)
- [目录结构](#目录结构)
- [核心模块](#核心模块)
- [开发规范](#开发规范)
- [数据流程](#数据流程)

---

## 项目概述

Table Tool 是一个 **Cocos Creator 跨版本插件框架**，核心是 **Vue 数据编辑器**。

### 设计理念

```
╔════════════════════════════════════════════════════════════╗
║  关注点分离：框架层 vs 业务层                                ║
╠════════════════════════════════════════════════════════════╣
║  框架层（已完成）         →  开发者无需关心                  ║
║  - 版本检测                                                 ║
║  - 适配器工厂                                               ║
║  - v2/v3 API 适配                                          ║
║  - IPC 通信桥接                                             ║
║                                                             ║
║  业务层（开发重点）       →  开发者主要工作区                 ║
║  - Vue 编辑器 UI                                           ║
║  - 数据管理逻辑                                             ║
║  - 组件功能实现                                             ║
╚════════════════════════════════════════════════════════════╝
```

### 核心特性

- ✅ **四种运行模式**：Standalone / Electron / Cocos 2.x / Cocos 3.x
- ✅ **统一 API 接口**：框架自动适配不同运行环境
- ✅ **Vue 数据编辑器**：完整的表格数据管理系统
- ✅ **加密存储**：.table 二进制格式

---

## 技术栈

### 框架层
- **TypeScript** - 类型安全
- **适配器模式** - 跨版本兼容
- **工厂模式** - 动态创建适配器

### 业务层（Vue 编辑器）
- **Vue 2.x** - 渐进式框架（兼容 Cocos 2.x/3.x）
- **Vite** - 快速开发构建
- **TypeScript** - 类型定义

### 打包工具
- **Electron** - 独立桌面应用
- **electron-builder** - 打包 exe

---

## 架构设计

### 整体架构

```
┌─────────────────────────────────────────────────────────────┐
│                   Table Tool 插件                           │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  ┌───────────────────────────────────────────────────────┐  │
│  │                  Vue 数据编辑器                        │  │
│  │            (src/panels/vue-editor/)                   │  │
│  │                                                       │  │
│  │  ┌─────────────────────────────────────────────────┐  │  │
│  │  │ 统一 API 接口层 (api/index.ts)                  │  │  │
│  │  └─────────────────────────────────────────────────┘  │  │
│  │              │              │              │          │  │
│  │              ▼              ▼              ▼          │  │
│  │    ┌──────────────┐ ┌──────────────┐ ┌──────────────┐ │  │
│  │    │ standalone.ts│ │  cocos.ts    │ │ electron.ts  │ │  │
│  │    │ (浏览器API)   │ │  (IPC通信)   │ │  (IPC通信)   │ │  │
│  │    └──────────────┘ └──────────────┘ └──────────────┘ │  │
│  └───────────────────────────────────────────────────────┘  │
│                                                             │
│  ┌───────────────────────────────────────────────────────┐  │
│  │              主进程（Node.js 环境）                     │  │
│  │                (src/main.ts)                          │  │
│  │                                                       │  │
│  │  ┌─────────────────────────────────────────────────┐  │  │
│  │  │ 适配器工厂 (core/factory.ts)                    │  │  │
│  │  └─────────────────────────────────────────────────┘  │  │
│  │              │                        │                │  │
│  │              ▼                        ▼                │  │
│  │    ┌──────────────────┐    ┌──────────────────┐       │  │
│  │    │  v2 适配器       │    │  v3 适配器       │       │  │
│  │    │ (adapters/v2/)  │    │ (adapters/v3/)  │       │  │
│  │    └──────────────────┘    └──────────────────┘       │  │
│  └───────────────────────────────────────────────────────┘  │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### 关键设计点

#### 1. 环境分离

**主进程（Node.js）**：
- 位置：`src/main.ts` + `src/adapters/`
- 职责：处理 Cocos v2/v3 版本差异
- 能力：完整的 Node.js API + Cocos Editor API

**Vue 面板（iframe 浏览器环境）**：
- 位置：`src/panels/vue-editor/src/`
- 职责：UI 渲染和用户交互
- 限制：无法直接访问 Node.js 模块

#### 2. IPC 桥接

Vue 面板通过 IPC 调用主进程功能：

```typescript
// Vue 面板 (src/panels/vue-editor/src/api/cocos.ts)
function sendToMain(method: string, ...args: any[]) {
    // v3: Editor.Message.request('cc-table-tool', method, ...args)
    // v2: Editor.Ipc.sendToMain('cc-table-tool:' + method, ...args)
}

// 主进程 (src/main.ts)
export const messages = {
    'select-file': async (options) => {
        const adapter = getEditor(); // 使用适配器工厂
        return adapter.selectFile(options);
    }
};
```

#### 3. 平台 API 统一

```typescript
// src/panels/vue-editor/src/api/index.ts
export interface IEditorApi {
    readFile(path: string): Promise<string>;
    writeFile(path: string, content: string): Promise<void>;
    selectFile(options?: any): Promise<string | null>;
    // ...
}

// 不同平台实现
cocos.ts      → IPC 调用主进程适配器
electron.ts   → IPC 调用主进程
standalone.ts → 浏览器 FileSystemAccess API
```

---

## 目录结构

```
cc_table_tool/
├── src/                          # 主进程源代码
│   ├── main.ts                   # 插件入口
│   ├── index.ts                  # 统一导出
│   │
│   ├── core/                     # 🔒 框架层（已完成，开发者无需修改）
│   │   ├── interfaces.ts         # 跨版本抽象接口
│   │   ├── version-detector.ts   # 版本检测器
│   │   ├── factory.ts            # 适配器工厂
│   │   └── base-plugin.ts        # 基础插件类
│   │
│   ├── adapters/                 # 🔒 框架层（已完成，开发者无需修改）
│   │   ├── v2/                   # Cocos 2.x 适配器
│   │   │   ├── editor-adapter.ts
│   │   │   ├── panel-adapter.ts
│   │   │   ├── ipc-adapter.ts
│   │   │   ├── asset-adapter.ts
│   │   │   └── scene-adapter.ts
│   │   └── v3/                   # Cocos 3.x 适配器
│   │       ├── editor-adapter.ts
│   │       ├── panel-adapter.ts
│   │       ├── ipc-adapter.ts
│   │       ├── asset-adapter.ts
│   │       └── scene-adapter.ts
│   │
│   ├── bridge/                   # 🔒 框架层（已完成，开发者无需修改）
│   │   ├── cocos-bridge.ts       # Cocos 通信桥接
│   │   ├── types.ts              # 桥接类型定义
│   │   └── index.ts
│   │
│   ├── panels/                   # 面板定义
│   │   ├── default/              # 默认面板（加载 Vue 编辑器）
│   │   └── vue-editor/           # ⭐ Vue 数据编辑器
│   │       ├── src/              # Vue 源代码
│   │       │   ├── App.vue       # 主应用组件
│   │       │   ├── main.ts       # Vue 入口
│   │       │   │
│   │       │   ├── api/          # 🔑 平台适配层（统一 API）
│   │       │   │   ├── index.ts  # API 接口定义
│   │       │   │   ├── standalone.ts  # 浏览器实现
│   │       │   │   ├── cocos.ts       # Cocos IPC 实现
│   │       │   │   └── electron.ts    # Electron IPC 实现
│   │       │   │
│   │       │   ├── components/   # Vue 组件
│   │       │   │   ├── ConfigManage.vue    # 配置管理
│   │       │   │   ├── TableEditor.vue     # 表编辑器
│   │       │   │   ├── DataEditor.vue      # 数据编辑器
│   │       │   │   ├── FieldEditor.vue     # 字段编辑器
│   │       │   │   ├── FieldInput.vue      # 字段输入
│   │       │   │   ├── ArrayEditor.vue     # 数组编辑器
│   │       │   │   └── ObjectEditor.vue    # 对象编辑器
│   │       │   │
│   │       │   └── utils/        # 工具模块
│   │       │       ├── types.ts           # 类型定义
│   │       │       ├── dataManager.ts     # 数据管理
│   │       │       ├── fieldFactory.ts    # 字段工厂
│   │       │       ├── importHelper.ts    # 导入助手
│   │       │       ├── serializer.ts      # 序列化
│   │       │       └── importExamples.ts  # 导入示例
│   │       │
│   │       ├── vite.config.ts    # Vite 配置
│   │       ├── index.html        # HTML 入口
│   │       └── package.json      # Vue 项目配置
│   │
│   ├── shared/                   # 共享模块（主进程和面板共享）
│   │   └── cocos-utils.ts        # Cocos 工具函数
│   │
│   └── tools/                    # 编辑器工具类
│       └── Tools.ts              # 文件操作、同步等
│
├── electron/                     # Electron 独立打包
│   ├── main.js                   # Electron 主进程
│   ├── preload.js                # 预加载脚本
│   └── package.json              # Electron 配置
│
├── dist/                         # 编译输出
│   ├── v2/                       # v2 编译产物
│   ├── v3/                       # v3 编译产物
│   └── vue-editor/               # Vue 编辑器构建产物
│
├── package.json                  # 主配置（Cocos 3.x）
├── package.v2.json               # v2 专用配置
├── package.v3.json               # v3 专用配置
├── tsconfig.json                 # TypeScript 基础配置
├── tsconfig.v2.json              # v2 编译配置
├── tsconfig.v3.json              # v3 编译配置
├── README.md                     # 项目简介
├── 使用说明.md                   # 详细文档
└── 项目架构说明.md               # 本文档
```

### 图例说明
- 🔒 **框架层** - 已完成，开发者无需修改
- 🔑 **平台适配层** - 关键模块，统一不同运行环境
- ⭐ **业务层** - 开发者主要工作区

---

## 核心模块

### 1. 适配器工厂 (core/factory.ts)

**职责**：根据版本创建对应的适配器实例

```typescript
export class AdapterFactory {
    static getInstance(): IAdapterManager {
        const version = VersionDetector.detect();
        if (version === CocosVersion.V2) {
            return new V2AdapterManager();
        } else {
            return new V3AdapterManager();
        }
    }
}

// 使用方式
import { getEditor } from './index';
const editor = getEditor();  // 自动返回对应版本的适配器
editor.log('Hello');         // 统一接口
```

### 2. 版本检测器 (core/version-detector.ts)

**职责**：自动检测当前 Cocos Creator 版本

```typescript
export class VersionDetector {
    static detect(): CocosVersion {
        if (typeof Editor === 'undefined') {
            return CocosVersion.Unknown;
        }
        
        const version = Editor.App?.version || '';
        if (version.startsWith('2.')) {
            return CocosVersion.V2;
        } else if (version.startsWith('3.')) {
            return CocosVersion.V3;
        }
        
        return CocosVersion.Unknown;
    }
}
```

### 3. 数据管理器 (utils/dataManager.ts)

**职责**：管理表格数据的 CRUD 操作

```typescript
export class DataManager {
    // 表管理
    addTable(key: string, params: any): void
    updateTable(key: string, params: any): void
    deleteTable(key: string): void
    
    // 数据管理
    addData(tableKey: string, dataKey: string, data: any): void
    updateData(tableKey: string, dataKey: string, data: any): void
    deleteData(tableKey: string, dataKey: string): void
    
    // 导入导出
    importTableFromJson(): Promise<void>
    exportTableToJson(tableKey: string): Promise<void>
    exportAllTables(): Promise<void>
    
    // 持久化
    load(filePath: string): Promise<void>
    save(): Promise<void>
}
```

### 4. 字段工厂 (utils/fieldFactory.ts)

**职责**：创建不同类型的字段定义

```typescript
export function createField(type: FieldType, params: any): IFieldDef {
    switch (type) {
        case 'string':
            return createStringField(params);
        case 'number':
            return createNumberField(params);
        case 'array':
            return createArrayField(params);
        case 'object':
            return createObjectField(params);
        // ...
    }
}

// 默认值获取
export function getDefaultValue(field: IFieldDef): any {
    // 统一处理所有字段类型的默认值
}
```

### 5. 序列化器 (utils/serializer.ts)

**职责**：数据加密和序列化

```typescript
// 序列化为加密二进制
export function serializeDataSource(dataSource: IDataSource): ArrayBuffer {
    const json = JSON.stringify(dataSource);
    const encrypted = encryptData(json);
    const buffer = compressData(encrypted);
    return addMagicHeader(buffer);
}

// 反序列化
export function deserializeDataSource(buffer: ArrayBuffer): IDataSource {
    validateMagicHeader(buffer);
    const decrypted = decryptData(buffer);
    const decompressed = decompressData(decrypted);
    return JSON.parse(decompressed);
}
```

---

## 开发规范

### 命名规范

```typescript
// 文件夹：驼峰命名 (camelCase)
src/panels/vueEditor/
src/utils/

// 文件名：大驼峰命名 (PascalCase)
DataManager.ts
FieldFactory.ts

// 类名：大驼峰命名 (PascalCase)
class DataManager {}
class FieldFactory {}

// 函数/方法：驼峰命名 (camelCase)
function handleCreateData() {}
async function importTableFromJson() {}

// 常量：全大写 + 下划线
const MAGIC_NUMBER = 0x5442_4C45;
const ENCRYPT_KEY = 'table_tool_2024';
```

### 代码拆分原则

- **单文件不超过 500 行**
- 按职责拆分模块
- 类型定义独立文件

示例：
```
dataManager.ts (928行) → 拆分为：
├── types.ts         (170行) - 类型定义
├── fieldFactory.ts  (100行) - 字段创建
├── importHelper.ts  (180行) - 导入逻辑
├── serializer.ts    (110行) - 序列化
└── dataManager.ts   (330行) - 主逻辑
```

### TypeScript 使用

```typescript
// ✅ 完整的类型定义
export interface IFieldDef {
    key: string;
    name: string;
    type: FieldType;
    defaultValue?: any;
}

// ✅ 类型安全的函数
export function createField(
    type: FieldType,
    params: IFieldParams
): IFieldDef {
    // ...
}

// ✅ 泛型使用
export function getDefaultValue<T extends IFieldDef>(field: T): any {
    // ...
}
```

---

## 数据流程

### 数据加载流程

```
1. 用户操作
   ↓
2. Vue 组件 (DataEditor.vue)
   ↓
3. 数据管理器 (dataManager.ts)
   ↓
4. 序列化器 (serializer.ts)
   ↓
5. API 适配层 (api/index.ts)
   ↓
6. 平台实现 (cocos.ts / electron.ts / standalone.ts)
   ↓
7. 文件系统
```

### 数据保存流程

```
1. 用户点击保存
   ↓
2. dataManager.save()
   ↓
3. serializeDataSource(dataSource)
   ├── JSON.stringify()
   ├── encryptData()      # 加密
   ├── compressData()     # 压缩
   └── addMagicHeader()   # 添加文件头
   ↓
4. api.writeBinaryFile(path, buffer)
   ↓
5. 写入 .table 文件
```

### 导入流程

```
1. 选择 JSON 文件
   ↓
2. importHelper.parseJson()
   ├── 判断数据格式（对象/数组）
   ├── 推断字段类型
   └── 生成表定义
   ↓
3. dataManager.addTable()
   ↓
4. dataManager.forceRefresh()  # 触发 UI 更新
   ↓
5. Vue 响应式更新
```

---

## 开发指南

### 添加新功能的步骤

#### 1. 确定功能位置
- **编辑器功能** → `src/panels/vue-editor/src/components/`
- **数据处理** → `src/panels/vue-editor/src/utils/`
- **平台 API** → `src/panels/vue-editor/src/api/`
- **主进程** → `src/main.ts` + `src/adapters/`

#### 2. 创建组件/模块
```typescript
// 示例：添加新的字段类型 'date'

// 1. 更新类型定义 (utils/types.ts)
export type FieldType = 'string' | 'number' | 'boolean' | 'date' | ...;

export interface IDateField extends IBaseField {
    type: 'date';
    format?: string;  // 日期格式
}

// 2. 更新字段工厂 (utils/fieldFactory.ts)
export function createDateField(params: any): IDateField {
    return {
        type: 'date',
        key: params.key,
        name: params.name,
        format: params.format || 'YYYY-MM-DD',
        defaultValue: new Date().toISOString()
    };
}

// 3. 更新 FieldInput 组件
<template>
  <input
    v-if="field.type === 'date'"
    type="date"
    :value="formatDate(value)"
    @input="handleDateInput"
  />
</template>
```

#### 3. 测试
```bash
# 浏览器测试
npm run dev:editor

# Electron 测试
npm run dev:electron

# Cocos 插件测试
npm run build:v3
# 然后在 Cocos Creator 中重载插件
```

---

## 常见问题

### Q1: 为什么 Vue 面板不能直接使用适配器？

**A**: Vue 面板运行在 iframe 浏览器环境中，无法访问 Node.js 模块。适配器层位于主进程（Node.js），需要通过 IPC 通信桥接。

### Q2: 如何扩展新的平台 API？

**A**: 
1. 在 `api/index.ts` 的 `IEditorApi` 接口添加方法
2. 在 `standalone.ts`、`cocos.ts`、`electron.ts` 中实现
3. 若需 IPC，更新 `electron/preload.js` 和 `src/main.ts`

### Q3: 如何调试？

**A**:
- **浏览器模式**：使用 Chrome DevTools
- **Electron 模式**：启用 DevTools（在 main.js 中）
- **Cocos 模式**：使用 Cocos Creator 的开发者工具

### Q4: 如何优化大数据量？

**A**:
- 使用虚拟滚动（Virtual Scroll）
- 分页加载
- 数据懒加载
- Web Worker 处理大数据

---

## 参考资料

- [README.md](./README.md) - 项目简介
- [使用说明.md](./使用说明.md) - 详细使用文档
- [数据管理模块说明](./src/panels/vue-editor/src/utils/README.md)
- [Cocos Creator 2.x 文档](https://docs.cocos.com/creator/2.4/manual/)
- [Cocos Creator 3.x 文档](https://docs.cocos.com/creator/3.8/manual/)

---

**最后更新**：2026-02-07
**维护者**：oldP
